...
export default function CampaignStatusDesktop() {
  ...
  useEffect(() => {
    ...
    script.onload = () => { console.log("포트원 스크립트 동적 생성 완료!"); } // 태그가 정상적으로 추가되었는 지 확인을 위한 로그
  ...
  const donate = () => {
    ...
    if (donationAmountRef.current.value == "" || donationAmountRef.current.value <= 0) {
      alert("기부 금액을 입력해주세요.");
      return;
    }
  ...
  useEffect(() => { // setPaymentInfo 함수가 값을 바꾼 이후 결제가 이루어져야 해서 paymentInfo의 변화를 추적
    if (preventFirstRender) { // 첫 번째 값 변경(페이지 렌더링)일 경우는 실행하지 않게 처리
    ...
    IMP.request_pay(paymentInfo, rsp => {
      ...
      if (rsp.success) { // 결제 성공 시
        ...
        donateCampaign(campaignStatus.projectId, 1, parseInt(donationAmountRef.current.value), "card");
      } else { // 결제 실패 시
        console.log("결제 실패!");
      }
    });
  ...
  return (
    <div className = "campaign-desktop" ref = {statusRef} style = {{ top: `${defaultY + scrollY}px` }}>
      <div className = "campaign-title">{campaignStatus.title}</div>
      <div className = "progress-wrapper">
        <div className = "progress-text-wrapper">
          <div className = "progress-percent">
            {campaignStatus.donationRate}%
          </div>
          <div className = "campaign-d-day">{campaignStatus.daysLeft >= 0 ? 'D-' + campaignStatus.daysLeft : '기간만료'}</div>
        </div>
        <div className = "progress-bar-wrapper">
          <progress className = "progress-bar" value = {campaignStatus.accumulatedDonation} max = {campaignStatus.targetAmount} />
        </div>
      </div>
      <div className = "amount-wrapper">
        <div className = "current-amount">{campaignStatus.accumulatedDonation}원</div>
        <div className = "target-amount">{campaignStatus.targetAmount}원 목표</div>
      </div>
      ...
      <div className = "donate-btn" onClick = {() => setModalOpen(true)}>기부하기</div>

      <CampaignDonateModal isOpen = {modalOpen} onClose = {() => setModalOpen(false)}>
        {/* children */}
        <div>
          ...
          <div>
            ...
            <button id = "donate" onClick = {() => donate()}>기부하기</button>
          </div>
        </div>
      </CampaignDonateModal>
    </div>
