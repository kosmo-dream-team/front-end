...
const CampaignContext = () => {
  ...
  useEffect(() => { // 추천 댓글 클릭 시 해당 내용이 댓글 입력하는 곳에 업데이트 되는 기능
    const setCommentToRecommended = (recommend) => {
      document.getElementById("comment").innerText = recommend.target.innerText;
    }

    if (recommendRef1.current) recommendRef1.current.addEventListener('click', setCommentToRecommended);
    if (recommendRef2.current) recommendRef2.current.addEventListener('click', setCommentToRecommended);
    if (recommendRef3.current) recommendRef3.current.addEventListener('click', setCommentToRecommended);

    return () => {
      if (recommendRef1.current) recommendRef1.current.removeEventListener('click', setCommentToRecommended);
      if (recommendRef2.current) recommendRef2.current.removeEventListener('click', setCommentToRecommended);
      if (recommendRef3.current) recommendRef3.current.removeEventListener('click', setCommentToRecommended);
    };
  }, []);
      ...
      <div className = "category-list-wrapper">
        <div className = "category-list">
          {campaignStatus.categoryList.map((category, index) => (
            <div className = "category" key = {index}>#{category.categoryName}</div>
          ))}
        </div>
      </div>
        ...
        <div className = "donor-list">
          {campaignStatus.donorList.map((donor, index) => (
            <div className = "donor-wrapper" key = {index}>
              <div className = "donor-img" style = {{backgroundImage: 'url('+donor.profileImage+')'}} />
      ...
      <div className = "comment-section-wrapper">
        <div className = "commenting-section">
          ...
          <div className = "comment-write-wrapper">
            <div id = "comment" className = "comment" contentEditable data-text = "댓글 남기기" />
          ...
          <div className = "comment-recommend-wrapper">
            ...
            <div id = "recommend-1" className = "comment-recommend" ref = {recommendRef1}>응원합니다.</div>
            <div id = "recommend-2" className = "comment-recommend" ref = {recommendRef2}>우리 함께 합시다!</div>
            <div id = "recommend-3" className = "comment-recommend" ref = {recommendRef3}>함께 참여해주세요.</div>
          </div>
        </div>
        <div className = "comment-section">
          ...
          <div className = "comment-list-wrapper">
            <div className = "comment-list">
              ...
              campaignStatus.commentList.map((comment, index) => ( // 달린 댓글이 있다면 댓글 목록 출력
                <div className = "comment-wrapper" key = {index}>
                  <div className = "comment-img-wrapper">
                    <div className = "user-img" style = {{backgroundImage: 'url(/src/assets/img/'+comment.profileImage+')'}} />
